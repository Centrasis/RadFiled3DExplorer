...
class MainWindow(FileSystemEventHandler):
    ...
    def __init__(self, caption: str, size: Tuple[int, int] = (1024, 700)):
        ...
        self.app.layout = html.Div([
            html.Div([
                ...
                ], style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'height': '10%', 'width': '100%'}),
                html.Ul(id='file-list', style={'height': '75%', 'overflowY': 'scroll', 'listStyleType': 'none', 'padding': '0', 'margin': '0'}),
                html.Div([
                    html.Button('<--', id='prev-btn', style={'width': '45%', 'margin-right': '5%'}),
                    html.Button('-->', id='next-btn', style={'width': '45%'})
                ], style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'height': '10%'})
            ...
            html.Div([
                ...
                ], style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'width': '100%', 'height': '10%', 'padding': '10px'}),
                html.Div([
                    html.Div([
                        dcc.Graph(id='viewer-graph', style={'width': 'calc(100% - 50px)', 'height': '100%', 'float': 'left'}),
                        html.Div([
                            dcc.Slider(id='vertical-slider', vertical=True, min=0, max=100, step=1, value=50, marks={0: '0', 100: '100'})
                        ], style={'width': '50px', 'height': '100%', 'float': 'right', 'display': 'flex'})
                ...
                html.Div([
                    ...
                    dcc.Checklist(id='enable-vertical-slicing', options=[{'label': 'Enable vertical slicing', 'value': 'vertical-slicing'}], value=['vertical-slicing']),
                ], style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'width': '100%', 'height': '10%', 'padding': '10px'})
    ...
    def update_viewer_plot(self):
        #self.enqueue_gui_call(lambda: self.window["-BUSY-GIF-"].update(visible=True))
        #self.enqueue_gui_call(lambda: self.window["-VIEWER-PLOT-"].update(visible=False))
        ...
        #self.enqueue_gui_call(lambda: self.window["-VIEWER-PLOT-"].update(visible=True))
        ...
        #self.enqueue_gui_call(lambda: self.window["-BUSY-GIF-"].update(visible=False))
